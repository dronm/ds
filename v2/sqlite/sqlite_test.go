...
func TestPrimary(t *testing.T) {
	...
	dbConn, err := sql.Open(DRIVER_NAME, DB_FILE)
	if err != nil {
		t.Fatalf("sql.Open() failed: %v", err)
	}
	prov, err := ds.NewProvider(PROVIDER_ID, dbConn)
	if err != nil {
		t.Fatalf("NewProvider() failed: %v", err)
	}
	//cast to sqlite
	var sqlite ds.Storage
	var ok bool
	sqlite, ok = prov.(*SQLiteProvider)
	if !ok {
		t.Fatalf("error casting to type: *SQLiteProvider")
	}
	//aquire connection
