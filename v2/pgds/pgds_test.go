...
func TestPrimary(t *testing.T) {
	...
	prov, err := ds.NewProvider(PROVIDER_ID, getTestVar(t, ENV_PG_CONN), nil, nil)
	if err != nil {
...
func TestSecondary(t *testing.T) {
	...
	prov, err := ds.NewProvider(PROVIDER_ID, "", nil, map[string]string{"sec1": getTestVar(t, ENV_PG_CONN)})
	if err != nil {
		t.Fatalf("NewProvider() failed: %v", err)
	}
	//cast to pg
